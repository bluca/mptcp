 ...
 */
static void set_gateway_available(struct sock *sk)
{
	int i;
 ...
 */
static void mptcp_v4_add_lsrr(struct sock *sk, struct mptcp_rem4 *rem)
{
	int i, j, ret;
	char * opt = NULL;
	struct tcp_sock * tp = tcp_sk(sk);
	...
	if (i < MPTCP_GATEWAY_MAX_LISTS) {
		...
		memcpy(opt + 4 + (j * sizeof(rem->addr)), &rem->addr,
				sizeof(rem->addr));
 ...
 *  marked as unavailable and the subflow's fingerprint is set to 0.
 */
static void mptcp_v6_add_rh0(struct sock *sk, struct mptcp_rem6 *rem)
{
	int i, ret;
	char * opt = NULL;
	struct tcp_sock * tp = tcp_sk(sk);
	...
	if (i < MPTCP_GATEWAY_MAX_LISTS) {
		 ...
		 */
		memcpy(opt + 8, &rem->addr, sizeof(rem->addr));
		memcpy(&rem->addr, &mptcp_gws6->list[0][0].s6_addr, sizeof(mptcp_gws6->list[0][0].s6_addr));
